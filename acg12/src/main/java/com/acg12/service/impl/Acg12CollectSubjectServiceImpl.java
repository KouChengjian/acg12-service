...
public class Acg12CollectSubjectServiceImpl extends GenericServiceImpl<Acg12CollectSubjectEntity, Long> implements Acg12CollectSubjectService {
    ...
    public Acg12CharacterDto buildHasCollectToCharacter(Acg12CharacterDto acg12CharacterDto, long userId) {
        ...
        List<Acg12CollectSubjectEntity> collectSubjectList = findListByPage(parameter);
        Map<Integer, Acg12CollectSubjectEntity> collectSubjectMap = collectSubjectList.stream().collect(Collectors.toMap(Acg12CollectSubjectEntity::getRelevanceId, a -> a, (k1, k2) -> k1));
        Acg12CollectSubjectEntity acg12CollectSubjectEntity = collectSubjectMap.get(acg12CharacterDto.getCId());
        if (acg12CollectSubjectEntity != null) {
            acg12CharacterDto.setIsCollect(1);
        } else {
            acg12CharacterDto.setIsCollect(0);
        }
    ...
    public Acg12PersonDto buildHasCollectToPerson(Acg12PersonDto acg12PersonDto, long userId) {
        ...
        List<Acg12CollectSubjectEntity> collectSubjectList = findListByPage(parameter);
        Map<Integer, Acg12CollectSubjectEntity> collectSubjectMap = collectSubjectList.stream().collect(Collectors.toMap(Acg12CollectSubjectEntity::getRelevanceId, a -> a, (k1, k2) -> k1));
        Acg12CollectSubjectEntity acg12CollectSubjectEntity = collectSubjectMap.get(acg12PersonDto.getPId());
        if (acg12CollectSubjectEntity != null) {
            acg12PersonDto.setIsCollect(1);
        } else {
            acg12PersonDto.setIsCollect(0);
        }
