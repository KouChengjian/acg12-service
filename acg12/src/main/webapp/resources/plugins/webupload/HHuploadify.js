...
var HHuploadify = function () {
	function HHuploadify() {
		...
		var defaults = {
			...
			chooseText: '选择', // words on choose button
			...
			onSelectError: null,
            preProcessOptions:null,// 上传之前预处理options
			onUploadStart: null, // when a file upload start
			onUploadSuccess: function(file,res){
                res = JSON.parse(res)
                var selector = this.options.inputValue;
                $(selector).val(jumper.getPrefix(this.options.cdnSelector) + res.key)
            }, // when a file upload success
			...
			onRemoved: function () {
                $(this.options.container + ' .uploadify-choose-button').show()
                var selector = this.options.inputValue;
                $(selector).val('')
            }, // when remove a file in the list
	...
	_createClass(HHuploadify, [{
		...
		value: function init() {
            ...
            /**
			 * 初始化完options就开始初始化cdn下拉
             */
            var cdnSelector = options.cdnSelector;
            $(cdnSelector).html(
            	'<option value="QINIU_PUBLIC_TOKEN">公有</option>' +
                '<option value="QINIU_PROTECTED_TOKEN">受保护</option>' +
                '<option value="QINIU_PRIVATE_TOKEN">私有</option>'
			);
			...
			if (options.files instanceof Array && options.files.length > 0) {
				 ...
				 * 修改的时候默认隐藏
                 */
				$(options.container + ' .uploadify-choose-button').hide()
		...
		value: function uploadFileByIFrame(file) {
            ...
            if(options.preProcessOptions)
                _this6.invoke(options.preProcessOptions,options,file);
		...
		value: function uploadFileByXHR(file) {
			...
			//预处理options
			if(options.preProcessOptions)
            	_this7.invoke(options.preProcessOptions,options,file);
