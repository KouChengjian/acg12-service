...
<head>
    ...
    <title>${setting.siteName} - subjectOffprint管理</title>
                ...
                <form id="listForm" action="list.html" method="get">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="dataTables_wrapper form-inline">
                            <table id="Acg12SubjectOffprint_list"></table>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    $('#Acg12SubjectOffprint_list').bootstrapTable({
        method: 'get',
        ...
        columns: [
            ...
            {
                title: "parentSId", field: "parentSId", align: "center", valign: "middle", sortable: true, visible: false
            },
            {
    ...
    function queryParams(params) {  //配置参数
        var temp = {
            ...
            filter_parentSIds: $("#filter_parentSIds").val(),
            filter_parentSIde: $("#filter_parentSIde").val(),
    ...
    function initColumns() {
        ...
        [#list systemColumnList as systemColumn]
	        $('#Acg12SubjectOffprint_list').bootstrapTable('${systemColumn.columnType}', '${systemColumn.columnName}');
        [/#list]
        //  初始化搜索字段
    ...
    //页面跳转
    function refreshNum() {
        $('#Acg12SubjectOffprint_list').bootstrapTable('selectPage', $('#searchPageNum').val());
    };

    //搜索按钮
    $(".btn-primary-search").click(function () {
        $('#Acg12SubjectOffprint_list').bootstrapTable('selectPage', 1);
        $('#Acg12SubjectOffprint_list').bootstrapTable(
                "refresh",
                {
                    url: "listNew.json"
    ...
    function dels(val) {
        bootbox.confirm(message("admin.dialog.deleteConfirm"), function (result) {
            var rows = $.map($('#Acg12SubjectOffprint_list').bootstrapTable('getSelections'), function (row) {
                return row.id
            });
            var ids = "";
