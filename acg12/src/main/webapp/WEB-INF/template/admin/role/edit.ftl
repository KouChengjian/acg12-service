                                    ...
                                    <div class="col-sm-10">
                                            ...
                                            <div class="checkbox-inline i-checks">
                                                <input type="checkbox" class="i-checks"
                                                       [#if role.aclIds?seq_contains(root.id)]checked[/#if] name="ids"
                                                       value="${root.id}">${root.name}
                                            </div>
                                            <br>
		                                    [#list root.children as child]
		                                    	<div class="checkbox-inline i-checks" style="margin-left:30px;">
                                                    <input type="checkbox" class="i-checks ${root.id}" name="ids"
                                                           [#if role.aclIds?seq_contains(child.id)]checked[/#if]
                                                           value="${child.id}">${child.name}
                                                </div>
			                                    <br>
                                                [#list child.children as btn]
			                                    	<div class="checkbox-inline i-checks"
                                                         [#if btn_index == 0]style="margin-left:60px;"[/#if]>
                                                        <input type="checkbox" class="i-checks" name="ids"
                                                               value="${btn.id}"
                                                               [#if role.aclIds?seq_contains(btn.id)]checked[/#if]>${btn.name}
                                                    </div>
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <div class="checkbox-inline i-checks">
                                            <input type="checkbox" class="i-checks" name="isSystem" value="true"
                                                   [#if role.isSystem == true]checked[/#if] >
                                        </div>
                                    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(".iCheck-helper").click(function () {
            var checkboxId = $(this).prev().val();
            var isChecked = $(this).prev().is(':checked');
            var checkboxs = $("." + checkboxId);
